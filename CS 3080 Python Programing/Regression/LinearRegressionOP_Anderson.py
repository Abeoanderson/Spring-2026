...
c = y_mean - m * x_mean
# A.1
print(f"Optimal coefficients found using math: m = {m},c = {c}")
# A.2
c = 0.0
m = 0.0
...
for i in range(epochs):
    y_pred = m * x + c
    # A.4
    dm = np.mean(-2 * x * (y - y_pred))
    dc = np.mean(-2 * (y - y_pred))
    # A.5
    m = m -lr * dm
    c = c - lr * dc
    # A.6
    if i % 100 == 0:
        ...
        print(f"Epoch {i}, MSE: {mse:.4f}, m:{m:.4f}, c: {c:.4f}")

# A.7
print(f"Optimal coefficients after optimization (training): m = {m}, c = {c}")
