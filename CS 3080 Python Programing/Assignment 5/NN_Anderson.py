...
def save_results(train_df, test_df, y_pred_train, y_pred_test, target, prefix):
    ...
    if isinstance(target, list):
        ...
        for i, item in enumerate(target):
            ...
            train_df["Predicted " + item] = (
                y_train_df["Predicted"] == i
            ).astype(int)

            test_df["Predicted " + item] = (
                y_test_df["Predicted"] == i
            ).astype(int)
            ...
            print(f"Training Accuracy for {item}: {train_accuracy:.2f}%")
            print(f"Testing Accuracy for {item}: {test_accuracy:.2f}%")
    else:
    ...
    train_df.to_csv(prefix + "_train_predictions.csv", index=False)
    test_df.to_csv(prefix + "_test_predictions.csv", index=False)
